...
class RestaurantManager:
    ...
    def get_sales_analysis(self) -> pd.DataFrame:
        ...
        try:
            ...
            if len(orders_df) == 0:
                return pd.DataFrame(columns=['order_id', 'customer', 'item', 'price'])
            
            # Explode the items column to get individual items
            items_df = orders_df.explode('items').reset_index(drop=True)
            
            # Convert items dictionary to separate columns
            items_details = pd.json_normalize(items_df['items'])
            
            # Combine order details with item details
            result_df = pd.concat([
                items_df.drop(['items', 'total', 'timestamp'], axis=1),
                items_details
            ], axis=1)
